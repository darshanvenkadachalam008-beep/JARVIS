...
Run once after cloning/updating:

```bash
python setup_security.py
```

This sets up three independent protections:

| Module | What it does |
|---|---|
| `core/secure_vault.py` | Encrypts every API key / Firebase credential at rest (AES via Fernet, PBKDF2-derived key, OS keyring integration) instead of leaving them in plaintext `config/api_keys.json`. |
| `core/access_control.py` | A separate PIN required before destructive actions run (currently gates the Telegram `/wipe` command — send `/wipe CONFIRM <PIN>`). Exponential lockout on repeated wrong guesses. |
| `core/audit_log.py` | Tamper-evident, hash-chained log (`memory/audit_log.jsonl`) of every security-relevant event — alerts, PIN attempts, vault unlocks. Editing or deleting a past entry breaks the chain and is detectable via `AuditLog().verify()`. |
| `core/command_guard.py` | Classifies any real (non-sandboxed) command as SAFE / needs-confirmation / BLOCKED before `computer_control.py`/`desktop.py` run it — a tripwire against catastrophic commands (disk format, `rm -rf /`, firewall disable, etc.) ever running unattended. |

`setup_security.py` will migrate your existing plaintext secrets into the vault and offer to delete the originals once you've confirmed the vault works. See the docstring at the top of each module for details and usage examples.

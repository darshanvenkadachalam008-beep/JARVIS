...
def browser_control(
    ...
    target_url = (parameters or {}).get("url", (parameters or {}).get("query", ""))
    ...
    try:
        ...
        guard(
            command=f"browser_control:{action} {target_url}",
            confirmed=confirmed,
            pin=pin,
            action_name=action,
        ...
        return f"Browser action refused by security guard: {pe}"
    ...
    try:
        if action == "go_to":
            ...
            raw_text = _bt.run(_bt._get_text())
            from core.taint_tracker import tag_untrusted_content
            result = tag_untrusted_content(raw_text, origin="browser_dom")

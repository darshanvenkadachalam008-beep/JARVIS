...
def desktop_control(
    ...
    try:
        ...
        target = params.get("path") or params.get("url") or task or ""
        confirmed = str(params.get("confirmed", "")).lower() in ("true", "1", "yes", "confirm") or bool(params.get("confirmed", False))
        pin = str(params.get("pin", ""))
        guard(
            command=f"desktop_control:{action or 'task'} {target}",
            confirmed=confirmed,
            pin=pin,
            action_name=action or "desktop_task",
            target=str(target),
        )
    except PermissionError as pe:
        return f"Desktop action refused by security guard: {pe}"

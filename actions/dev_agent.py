...
# hang indefinitely (no upstream default timeout), which previously caused
# JARVIS to go silent until the chat WebSocket itself timed out and reset.
...
def _run_project(run_command: str, project_dir: Path, timeout: int = 30,
    ...
    """
    PHASE 5 FIX: previously ran the LLM-generated project with a bare
    subprocess.run(..., cwd=project_dir) — same interpreter, same
    filesystem/network access, same environment (including JARVIS's own
    API keys) as the host. Now routed through code_sandbox.run_sandboxed(),
    which adds a clean/secret-stripped environment, POSIX resource limits
    (CPU/memory/process-count) where available, and a static pre-flight
    scan over the generated source for the handful of patterns that have
    no legitimate reason to appear in normal generated code (recursive
    deletes of root/home, shell=True, etc). source_files lets the caller
    pass in everything Tom wrote so the scan covers the whole project, not
    just the entry point.
    """
    from actions.code_sandbox import run_sandboxed

    print(f"[DevAgent] 🚀 Running (sandboxed): {run_command}")
    ...
    result = run_sandboxed(
        ...
        timeout=timeout,
        source_files_to_scan=source_files,
    )

    if result.blocked:
        return (
            f"⚠️ Run blocked by safety check: {result.blocked_reason}.\n"
            f"The project files are saved at {project_dir} — please review "
            f"and run them yourself if you're confident they're safe."
        )
    if result.timed_out:
...
def _build_project(
    ...
    for attempt in range(1, MAX_FIX_ATTEMPTS + 1):
        ...
        last_output = _run_project(run_command, project_dir, timeout,
                                    source_files=list(file_codes.values()))
...
def dev_agent(
    ...
    # ── Zero-Trust Gate: Command Guard Check ─────────────────────────────
    try:
        from core.command_guard import guard
        confirmed = str(p.get("confirmed", "")).lower() in ("true", "1", "yes", "confirm") or bool(p.get("confirmed", False))
        pin = str(p.get("pin", ""))
        guard(
            command=f"dev_agent build {project_name or description[:30]}",
            confirmed=confirmed,
            pin=pin,
            action_name="dev_agent",
            target=project_name or description[:50],
        )
    except PermissionError as pe:
        return f"Dev agent build refused by security guard: {pe}"

...
def _wmi_brightness_delta(delta: int):
    """Direct in-process WMI call for laptop-panel brightness — no
    subprocess/PowerShell spawn, so this is milliseconds instead of the
    1-3+ seconds a fresh `powershell -Command "Get-WmiObject ..."` call
    costs. Uses pywin32 (already a dependency for JARVISWatcherService),
    not a new package. Silently no-ops (fast) if the display doesn't
    expose the WmiMonitorBrightness class at all — common on desktop/
    external monitors, which don't support this API regardless of how
    it's called."""
    try:
        import pythoncom
        ...
        try:
            ...
            cur = wmi.ExecQuery("SELECT * FROM WmiMonitorBrightness")
            current = None
            for item in cur:
                current = item.CurrentBrightness
                break
            if current is None:
                return  # this display doesn't support WMI brightness control
            new_val = max(0, min(100, current + delta))
            methods = wmi.ExecQuery("SELECT * FROM WmiMonitorBrightnessMethods")
            for item in methods:
                item.WmiSetBrightness(1, new_val)
                break
        finally:
            pythoncom.CoUninitialize()
    except Exception as e:
        print(f"[Settings] WMI brightness call failed: {e}")
...
def lock_screen():
    if _OS == "Windows":
        # NOTE: simulating the Win+L keystroke (pyautogui.hotkey("win","l"))
        # does NOT reliably lock Windows — since Vista, the OS silently
        # ignores synthetic/programmatic Win+L key injection specifically,
        # as an anti-malware protection against fake-keystroke lock attacks.
        # That call would run with no error, but the screen just wouldn't
        # lock. LockWorkStation() is the actual, documented, reliable API
        # for this and isn't subject to that block.
        try:
            import ctypes
            ok = ctypes.windll.user32.LockWorkStation()
            if not ok:
                raise OSError("LockWorkStation returned failure")
        except Exception:
            # Fallback only if the direct API call itself couldn't be made
            # (e.g. ctypes unavailable for some reason).
    ...
    if action in _DANGEROUS_ACTIONS:
        ...
        if confirmed not in ("yes", "true", "1", "confirm"):
            return (
                ...
                f"Please confirm by calling again with confirmed=yes and pin=<your security PIN>."
            )
        # Second factor: a "confirmed=yes" flag alone can come from a spoofed tool
        # call, a compromised mobile session, or an injected voice command - none of
        # which prove a human actually approved this. Require the same PIN used to
        # gate the emergency wipe (core/access_control.py). Every attempt, pass or
        ...
        try:
            ...
            if not ac.is_configured():
                return (
                    f"{action} refused: no security PIN configured yet. "
                    f"Run setup_security.py once to enable this action."
                )
            if not ac.verify_pin(pin, action=f"computer_settings:{action}"):
                return f"{action} refused: PIN missing or incorrect. This attempt was logged."
        except ImportError:
            return f"{action} refused: core.access_control not available."

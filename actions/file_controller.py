...
def delete_file(path: str, confirm: bool = True, pin: str = "") -> str:
    """
    Deletes a file or folder.
    Moves to Recycle Bin on Windows if possible (reversible, no PIN needed).
    If send2trash is unavailable, falls back to a PERMANENT, unrecoverable
    delete — that fallback now requires the same security PIN as the wipe
    command and restart/shutdown (core/access_control.py), since it is the
    one destructive path here with no undo at all.
    """
    try:
        target = Path(path).expanduser()
        if not target.exists():
        ...
        try:
            ...
            if not ac.is_configured():
                return (
                    ...
                    "security PIN is configured yet. Run setup_security.py, or "
                    "install send2trash for reversible deletes."
                )
            if not ac.verify_pin(pin, action="file_controller:permanent_delete"):
                return "Permanent delete refused: PIN missing or incorrect. This attempt was logged."
        except ImportError:
            return "Permanent delete refused: core.access_control not available."
...
def read_file(path: str, max_chars: int = 3000) -> str:
    ...
    try:
        ...
        from core.taint_tracker import tag_untrusted_content
        return tag_untrusted_content(content, origin=f"file:{target.name}")
...
def file_controller(
    ...
    # ── Zero-Trust Gate: Command Guard & Authorization Tier Check ─────────
    try:
        from core.command_guard import guard
        target_path = _full_path(path, name)
        confirmed = str((parameters or {}).get("confirmed", "")).lower() in ("true", "1", "yes", "confirm") or bool((parameters or {}).get("confirmed", False))
        pin = str((parameters or {}).get("pin", ""))
        guard(
            command=f"file_controller:{action} {target_path}",
            confirmed=confirmed,
            pin=pin,
            action_name=action,
            target=target_path,
        ...
        return f"File operation refused by security guard: {pe}"
    ...
    try:
        if action == "list":
            ...
            full = _full_path(path, name)
            result = delete_file(full, pin=str(parameters.get("pin", "")))

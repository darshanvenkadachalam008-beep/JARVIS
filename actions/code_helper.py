...
# request or an overloaded model can hang indefinitely — there's no
# upstream timeout by default, and that previously caused JARVIS to go
# silent until the chat WebSocket itself timed out and reset.
...
def _read_file(file_path: str) -> tuple[str, str]:
    ...
    try:
        ...
        guard(
            command=f"code_helper:read {file_path}",
            action_name="read",
            target=file_path,
        )
    except PermissionError as pe:
        return "", f"Security violation: {pe}"
    p = Path(file_path)
    if not p.exists():
...
def _save_file(path: Path, content: str) -> str:
    try:
        from core.command_guard import guard
        guard(
            command=f"code_helper:write {path}",
            action_name="write_file",
            target=str(path),
        )
    except PermissionError as pe:
        return f"Save refused by security guard: {pe}"
    try:
...
def _run_file(path: Path, args: list, timeout: int) -> str:
    """
    PHASE 5 FIX: previously a bare subprocess.run(interp + [path], ...) —
    same interpreter/environment/permissions as JARVIS itself, and this is
    reachable both from Tom's own "write it and run it" flow AND from a
    user voice command asking to run an arbitrary existing file by path
    (code_helper's "run" action), so `path.parent` here isn't guaranteed
    to be a throwaway project folder the way dev_agent's is. Routed through
    the same code_sandbox.run_sandboxed() used by dev_agent — clean/secret-
    stripped environment, POSIX resource limits where available, and a
    static pre-flight scan of the file's own source before it's ever run.
    """
    ...
    try:
        ...
        source = ""   # if we can't even read it, let the run attempt surface why

    result = run_sandboxed(
        command=interp + [str(path)] + (args or []),
        work_dir=path.parent,
        timeout=timeout,
        source_files_to_scan=[source],
    )

    if result.blocked:
        return (
            f"⚠️ Run blocked by safety check: {result.blocked_reason}. "
            f"Please review {path.name} manually before running it yourself."
        )
    if result.timed_out:

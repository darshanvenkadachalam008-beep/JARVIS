...
def _call_tool(tool: str, parameters: dict, speak: Callable | None) -> str:
    # ── Zero-Trust Gate: Command Guard & Tier Enforcement ────────────────
    from core.command_guard import guard
    params = parameters or {}
    action_name = params.get("action", tool)
    target = (
        params.get("path")
    ...
    confirmed = str(params.get("confirmed", "")).lower() in ("true", "1", "yes", "confirm") or bool(params.get("confirmed", False))
    confirmed_action = str(params.get("confirmed_action", params.get("confirmed_action_name", "")))
    pin = str(params.get("pin", ""))

    # Evaluate guard check before tool dispatch
    guard(
        command=params.get("command", f"{tool}:{action_name}"),
        confirmed=confirmed,
        pin=pin,
        action_name=action_name,
        target=str(target),
        confirmed_action_name=confirmed_action if confirmed_action else (action_name if confirmed else None),
    )
